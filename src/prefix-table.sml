...
structure PrefixTable :> PREFIX_TABLE = struct
    ...
    type t = { forward : string StringMap.map,
               reverse : string StringMap.map,
               flat : (string * string) list }
    type iri = Iri.t
    ...
    val empty : t = { forward = StringMap.empty,
                      reverse = StringMap.empty,
                      flat = [] }
	...
	case StringMap.find (forward, namespace) of
	    NONE => table
	  | SOME expansion =>
	    { forward = remove_from (forward, namespace),
	      reverse = remove_from (reverse, expansion),
              flat = sorted_flat
                         (List.filter (fn (ns, _) => ns <> namespace) flat) }
    ...
    fun add (table : t, namespace, expansion) =
        (* remove the namespace first -- adding to a map will replace
           ...
           flat list *)
        let val table = remove (table, namespace) in
	    { forward = StringMap.insert (#forward table, namespace, expansion),
	      reverse = StringMap.insert (#reverse table, expansion, namespace),
              flat = sorted_flat ((namespace, expansion) :: (#flat table)) }
        end
	...
	isSome (StringMap.find (#forward table, namespace))
    ...
    fun expand (table, "a") = RdfStandardIRIs.iri_rdf_type
      | expand (table : t, curie) =
	    ...
	    (case String.fields (fn x => x = #":") curie of
	         [] => curie
	       | namespace::rest =>
	         case StringMap.find (#forward table, namespace) of
		     NONE => curie
    ...
    fun abbreviate_matching (table : t, iristr) =
        ...
        case List.find (fn (_, e) => Comp.is_prefix (e, iristr)) (#flat table) of
            NONE => NONE
          | SOME (ns, e) => 
	...
	let fun match_prefix_of "" = (0, "")
		...
		let val len = String.size name in
		    case StringMap.find (#reverse table, name) of
	...
	else abbreviate_reducing (table, Iri.toString iri)
